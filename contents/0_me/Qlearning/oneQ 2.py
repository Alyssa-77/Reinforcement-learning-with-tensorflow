...
def getData(data):
    ...
    if data["type"] == "perf":      # get perf
        ...
        print("state: bw=",s['b'],"loss=",s['o'],"delay=",s['d'],"tput=",s['p'])
        if FIRST==True:
            r = 0
            a = "0x10000"
        else:
            r = calReward(s, ACTION)
            print("reward:", r)
...
def calReward(s, a):
    # print("reward")
    z = zscore_fix(s['o'], s['d'], s['p'], wnd=a) # z=[zo, zd, zp] wnd="0x10000"  todo 每輪更新
    print("z=",z[0],z[1],z[2])
    r = round(W_LOSS*z[0]+W_DELAY*z[1]+W_TPUT*z[2], 3)   # (-0.5loss, -0.5delay, 0tput)
    return r
...
def zscore_fix(o, d, p, wnd): # 固定值下去算，todo 每輪更新
    ...
    zo = (o-avg[wnd_index][0])/sd[wnd_index][0] # Z=(x-avg)/sd
    zd = (d-avg[wnd_index][1])/sd[wnd_index][1]
    zp = (p-avg[wnd_index][2])/sd[wnd_index][2]
    return [zo, zd, zp]
...
# Main function =============================================================
EP = 3         # RL run ep times
MAKE_SEC = 16   # C make second

LOG = "/home/yuki/myquic/msquic/myperf/oneQ/state.txt"
...
for i in range(EP): # todo while true or not done
    if i==0:
        ...
        resetTxt(LOG)   # reset state.txt
    else:
        FIRST=False

    subprocess.Popen(["sudo", "xterm", "-e", STREAM[0]]) 
    print("\nep:",i+1,", wait for make:", MAKE_SEC,"s")
